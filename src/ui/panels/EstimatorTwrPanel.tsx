...
export function EstimatorTwrPanel(): React.JSX.Element | null {
  ...
  const { directional, thrusters, planet } = result;
      ...
      <div className="flex flex-col gap-4">
        {noGravity ? (
          <div className="flex items-center gap-2 rounded-lg border border-border bg-bg p-3 text-sm text-muted">
            No gravity here — any thrust holds. TWR is not applicable in space.
          </div>
        ) : (
          <div className="flex items-baseline justify-between">
            ...
            <span
              className={cn('font-mono text-xs', upTwr >= 1 ? 'text-success' : 'text-danger')}
            >
              up {formatTwr(upTwr)}
            </span>
          </div>
