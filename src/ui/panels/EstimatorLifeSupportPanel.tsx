/**
 * EstimatorLifeSupportPanel — crew oxygen balance for the *estimated* build.
 *
 * The Estimate-mode counterpart to Analyze's LifeSupportPanel: it runs the same
 * trusted `lifeSupport` engine, but on the synthesized {@link ShipDesign} from
 * `useEstimate` (via `estimateToDesign`) instead of an imported blueprint. This
 * lets a user declare an O2/H2 generator + oxygen tank as essentials and see
 * whether the build breathes — before a ship exists to export.
 *
 * Mirrors EstimatorTwrPanel's contract: renders nothing until there are
 * essentials to size against, and further self-hides its body via the engine's
 * empty state when the build has no gas gear (so an essentials-only build stays
 * uncluttered rather than showing a wall of zeros).
 */
import { useState } from 'react';
import { lifeSupport } from '@core';
...
export function EstimatorLifeSupportPanel(): React.JSX.Element | null {
  ...
  const ls = lifeSupport(result.design, { crewSize });
  const {
    hasLifeSupport,
    oxygenGeneration,
    oxygenCapacity,
  ...
  // Empty state: no generators and no oxygen tanks to reason about. Since the
  // estimator never places gas gear on its own, this is the common case until
  // the user declares an O2/H2 generator or tank as an essential.
  if (!hasLifeSupport) {
    return (
        ...
        <div className="flex items-start gap-3 rounded-lg border border-border bg-bg p-3 text-sm">
          ...
          <div>
            ...
            <p className="text-muted">
              Add an O2/H2 generator and an oxygen tank to your essentials to see crew oxygen
              balance, breathing time, and ice burn.
            </p>
          </div>
        </div>
      </Panel>
    ...
    >
      <div className="flex flex-col gap-4">
          ...
          >
            ...
            <div className="text-sm">
              ...
              <p className="text-muted">
                ...
                {oxygenCapacity > 0
                  ? `, lasting ${formatDuration(breathingTimeSeconds)}.`
                  : ' (and there is none stored).'}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          />
          <Stat label="Ice burn" value={formatFlow(iceBurnForOxygen)} hint="to sustain O₂ output" />
        </div>
      </div>
    </Panel>
  );
